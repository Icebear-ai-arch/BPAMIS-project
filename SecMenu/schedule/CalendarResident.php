<?php
// CalendarResident: Resident hearings calendar embedded via iframe.
// Normalize session adoption & user id retrieval (supports legacy Resident_ID).// adopts role-specific session name if cookie present
require_once __DIR__ . '/db-connect.php';
include_once __DIR__ . '/../../server/server.php'; // shared mysqli $conn

// Fallback: if user_id missing but Resident_ID exists, promote it.
if (empty($_SESSION['user_id']) && !empty($_SESSION['Resident_ID'])) {
  $_SESSION['user_id'] = $_SESSION['Resident_ID'];
}

// Guard login
if (empty($_SESSION['user_id'])) {
  error_log('[CalendarResident] No user_id in session; redirecting to login');
...
if ($val->num_rows == 0) {
  error_log('[CalendarResident] Resident_ID '. $Resident_ID .' not found; forcing logout');
  echo "<script>alert('Resident not found.'); window.location.href='../../bpamis_website/login.php';</script>";
  exit;
}

$events = [];
$hearings_list = [];
// Sync: hide any schedule rows that already have meeting logs (preserve history but hide from calendars)
$sync_sql = "
...
// Select visible hearings for this resident, excluding hearings that already have meeting logs
$hearing_stmt = $conn->prepare("
  SELECT DISTINCT sl.hearingID, sl.hearingTitle, sl.hearingDateTime, sl.remarks, ci.Case_ID
  ...
  LEFT JOIN barangay_case_management.complaint_respondents cr ON co.Complaint_ID = cr.Complaint_ID
  LEFT JOIN barangay_case_management.MEETING_LOGS ml ON ml.Case_ID = sl.case_id
    AND DATE(ml.Hearing_Date) = DATE(sl.hearingDateTime)
    AND TIME(ml.Hearing_Time) = TIME(sl.hearingDateTime)
  WHERE (
    co.Resident_ID = ? 
    OR COALESCE(co.respondent_id, co.Respondent_ID) = ?
    OR cr.Respondent_ID = ?
  )
  AND sl.visible = 1
  AND ml.Log_ID IS NULL
  AND TRIM(LOWER(COALESCE(ci.Case_Status, ''))) NOT IN (
    'mediation resolved',
...
");
$hearing_stmt->bind_param("iii", $Resident_ID, $Resident_ID, $Resident_ID);
...
$case_stmt->execute();
$case_res = bpamis_stmt_get_result($case_stmt);
...
<head>
  ...
  <style>
    ...
    /* Three-dot menu for mobile view buttons */
    #viewMenuBtn {
      display: none;
      position: relative;
      background: linear-gradient(120deg, #e8f7ff 0%, #f3fbff 100%) !important;
    ...
    #viewMenuDropdown {
      ...
      border: 1px solid #e0e7ef;
      border-radius: 10px;
      box-shadow: 0 8px 24px rgba(2, 129, 212, 0.18);
      backdrop-filter: blur(8px);
      z-index: 1000;
      min-width: 120px;
      overflow: hidden;
    }
    #viewMenuDropdown.show {
      display: block;
    ...
    #viewMenuDropdown button:hover {
      background: linear-gradient(120deg, #e8f7ff 0%, #f3fbff 100%) !important;
      color: #0369a1 !important;
    }
    #viewMenuDropdown button.active-view {
      background: #e4f6ff !important;
      color: #0b6fa2 !important;
      font-weight: 700;
    }
    ...
    @media (max-width: 768px) {
      #viewMenuBtn { display: inline-flex; }
      .fc .fc-toolbar-chunk:nth-child(3) .fc-button-group { display: none !important; }
      /* Position the menu button in the left chunk with prev/next buttons */
      .fc .fc-toolbar-chunk:nth-child(1) { gap: 0.5rem; }
    }
  ...
  const hearingsData = <?= $hearings_json ?>;
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
          ...
          eventClick: function (info) {
            ...
            // Send message to parent window to show modal
        ...
        // Three-dot menu for mobile view switching
        // Create and insert the menu button with prev/next buttons
        const viewMenuBtn = document.createElement('div');
        viewMenuBtn.id = 'viewMenuBtn';
        viewMenuBtn.className = 'fc-button';
        ...
        // Insert the button into the right toolbar chunk so it appears at the upper-right on mobile
        const rightChunk = document.querySelector('.fc-toolbar-chunk:nth-child(3)');
        if (rightChunk) {
          // append after any existing groups so it sits on the far right
          rightChunk.appendChild(viewMenuBtn);
        }
        ...
        if (viewMenuBtn && viewMenuDropdown) {
          ...
          document.addEventListener('click', function(e) {
            if (!viewMenuBtn.contains(e.target)) {
              viewMenuDropdown.classList.remove('show');
            }
          });

          // Handle view button clicks
          viewMenuDropdown.querySelectorAll('button').forEach(btn => {
            btn.addEventListener('click', function(e) {
              ...
              if (view && calendar) {
                ...
                // Update active state
                viewMenuDropdown.querySelectorAll('button').forEach(b => b.classList.remove('active-view'));
          ...
          const initialView = calendar.view.type;
          const activeBtn = viewMenuDropdown.querySelector(`button[data-view="${initialView}"]`);
          if (activeBtn) activeBtn.classList.add('active-view');

          // Update active view when calendar view changes
        ...
        function getPhaseColor(phase) {
          ...
          if (p.includes('conciliation')) return '#10b981'; // emerald
          if (p.includes('arbitration')) return '#3b82f6'; // blue
