...
<body>
  ...
  <!-- Local event modal (used when CalendarSec is opened directly, not embedded in a parent) -->
  ...
  <?php 
    ...
    // Exclude hearings for cases that are already resolved/dismissed/CTFA
    // AND exclude hearings that already have meeting logs recorded
  ...
  eventDidMount: function(info){
          ...
          if (iconEl) {
            iconEl.addEventListener('click', function(e){
              ...
              // Meeting log link for this scheduled hearing (assumes meeting_cases_log.php accepts case_id & hearing_id)
              const meetingLogLink = `/BPAMIS/SecMenu/meeting_cases_log.php?case_id=${ev.Case_ID}&hearing_id=${ev.hearingID}`;
              const contentHTML = `
                ...
                <p class="mb-2"><strong class="text-gray-700">Meeting Log:</strong> <a href="${meetingLogLink}" target="${targetAttr}" class="text-emerald-700 hover:text-emerald-800 font-semibold text-sm underline">Minutes of the Meeting</a></p>
  ...
  setTimeout(()=>calendar.updateSize(), 50);
    ...
    // Open the in-page local modal used when this calendar is opened directly (Secretary view)
    function openLocalEventModal(payload){
      try {
        var wrapper = document.getElementById('eventModalLocal');
        var content = document.getElementById('eventModalLocalContent');
        if (!wrapper || !content) return;
        // Build modal inner HTML: header + provided content + action buttons for Secretary
        var title = payload && payload.title ? payload.title : 'Schedule Details';
        var eventId = payload && payload.eventId ? payload.eventId : '';
        var html = '';
        html += '<div class="p-6">';
        html += '<h3 id="modalLocalTitle" class="text-lg font-bold text-sky-700 mb-3">' + escapeHtml(title) + '</h3>';
        html += '<div id="modalLocalBody">' + (payload && payload.content ? payload.content : '') + '</div>';
        html += '<div class="mt-4 flex justify-end gap-2">';
        // Only Secretary (this file lives inside SecMenu) should see the Delete button — use a simple link to server delete endpoint
        if (eventId) {
          var delUrl = 'delete_schedule.php?id=' + encodeURIComponent(eventId) + '&return=CalendarSec.php';
          html += '<button id="confirmDeleteLocal" class="px-4 py-2 bg-rose-600 text-white rounded-md font-semibold hover:opacity-95">Delete</button>';
        }
        html += '<button id="close-eventModalLocal-btn" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md font-semibold">Close</button>';
        html += '</div>';
        html += '</div>';
        ...
        // Wire buttons
  var closeBtn = document.getElementById('close-eventModalLocal-btn');
  if (closeBtn) closeBtn.addEventListener('click', function(){ wrapper.classList.add('hidden'); });
  // also wire existing close button placed in modal markup (if present)
  var existingClose = document.getElementById('close-eventModalLocal');
  if (existingClose) existingClose.addEventListener('click', function(){ wrapper.classList.add('hidden'); });

        var delBtn = document.getElementById('confirmDeleteLocal');
        if (delBtn) {
          delBtn.addEventListener('click', function(){
            if (!eventId) return;
            if (!confirm('Are you sure you want to delete this hearing schedule? This will hide it from participant calendars.')) return;
            // Navigate to server-side deletion endpoint which will perform the DB update and redirect back
            window.location.href = delUrl;
          });
        }
        // allow clicking outside modal content to close
        wrapper.addEventListener('click', function(ev){ if (ev.target === wrapper) wrapper.classList.add('hidden'); });
