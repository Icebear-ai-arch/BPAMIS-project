...
$query = "SELECT first_name, last_name FROM {$TB_RESIDENT_INFO} WHERE resident_id = ?";
...
if ($resident_id > 0) {
    $sql = "SELECT COUNT(*) as total 
            ...
            WHERE n.resident_id = $resident_id AND n.is_read = 0 AND t.notification_id IS NULL";
    $result = $conn->query($sql);
    if ($result && $row = $result->fetch_assoc()) {
        $unread_count = (int)$row['total'];
    }
}
// echo "Resident ID: " . $resident_id . " | Unread Count: " . $unread_count;
?>

<!-- Navigation Bar -->
<nav class="bg-white border-b border-blue-50 py-3 px-5 shadow-sm flex justify-between items-center sticky top-0 z-50">    
    <div class="flex items-center space-x-3">
    <a href="../ResidentMenu/home-resident.php"><img src="../SecMenu/logo.png" alt="Logo" class="w-10 h-10 rounded-full border-2 border-blue-200 drop-shadow-lg"></a>
    ...
    <div class="hidden md:block absolute left-1/2 transform -translate-x-1/2">
        <ul class="flex items-center space-x-6">
            <li>
                <a href="home-resident.php" class="tooltip relative group flex items-center justify-center p-3 rounded-full hover:bg-blue-50 transition-all duration-300" data-tooltip="Home">
            ...
            <li>
                <a href="notifications.php" class="tooltip relative group flex items-center justify-center p-3 rounded-full hover:bg-blue-50 transition-all duration-300" data-tooltip="Notifications">
                    <div class="relative">
                        ...
                        <span id="notif-count-badge" class="hidden absolute -top-1 -right-1 bg-red-500 text-white text-[10px] rounded-full h-4 min-w-[16px] px-1 flex items-center justify-center shadow-sm group-hover:scale-125 transition-transform" aria-hidden="true"></span>
    ...
    <div class="flex items-center space-x-5">   
        <!-- Mobile notification button (visible only below md) -->
        <a href="notifications.php" aria-label="Notifications" class="md:hidden relative p-2 rounded-full hover:bg-blue-50 transition text-gray-600">
            <div class="relative">
                <i class="fas fa-bell text-blue-500 text-lg"></i>
                <?php if ($unread_count > 0): ?>
                    <span class="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] rounded-full h-4 min-w-[16px] px-1 flex items-center justify-center shadow-sm"><?php echo $unread_count; ?></span>
                <?php endif; ?>
            </div>
        </a>
        ...
        <div class="relative group">
            <button class="flex items-center space-x-2 text-gray-700 hover:text-blue-600 transition-colors duration-300 rounded-full px-3 py-1 hover:bg-blue-50">
                ...
                <span class="hidden md:inline text-sm font-medium">Resident</span>
...
/* Hide the mobile menubar panel on small screens per request. The
     #mobile-menu element is only used for mobile (it has md:hidden), so
     hiding it here removes the menubar entirely on phones without
     affecting the desktop navbar. */
@media (max-width: 767px) {
    #mobile-menu { display: none !important; }
}
/* Make the nav fixed at the top on mobile so it remains visible while scrolling.
    Also add margin to the following element to prevent content being hidden
    beneath the fixed navbar. We only apply this for small screens. */
@media (max-width: 767px) {
  nav[role="navigation"], nav { position: fixed !important; top: 0; left: 0; right: 0; z-index: 999999 !important; width: 100%; }
  /* Approximate navbar height (py-3 plus content) — reserve space so page content
      doesn't sit under the fixed nav. Adjust the value if your nav's computed
      height differs. */
  nav + * { margin-top: 64px !important; }
}
</style>

<script>
    // Ensure favicon uses SecMenu/logo.png for resident pages
    (function setFavicon(){
        try {
...
<script>
    // Poll for new notifications (resident) and update badge without page refresh
    (function(){
        ...
        function renderBadge(count, isManualUpdate = false){
            const el = document.getElementById(BADGE_ID);
            if (!el) {
                console.log('Badge element not found:', BADGE_ID);
                return;
            }
            
            if (isManualUpdate) {
                console.log('Manual badge update to:', count);
                manuallyUpdated = true;
                clearTimeout(manualUpdateTimeout);
                manualUpdateTimeout = setTimeout(() => {
                    manuallyUpdated = false;
                    console.log('Manual update timeout expired, resuming polling');
            ...
            if (!count || count <= 0) {
                ...
                el.textContent = '0';
                console.log('Badge hidden (count:', count, ')');
            } else {
        ...
        window.updateResidentBadge = function(count) {
            console.log('updateResidentBadge called with count:', count);
            renderBadge(count, true);
        };
        ...
        document.addEventListener('DOMContentLoaded', function(){
            ...
            // Refresh badge when returning to page (bfcache) or when tab becomes visible
                ...
                // Cross-tab updates: BroadcastChannel + localStorage fallback
                try{
                    if ('BroadcastChannel' in window) {
                        const ch = new BroadcastChannel('bpamis-channel');
                        ch.onmessage = function(ev){ try{ var d = ev && ev.data ? ev.data : null; if(d && d.type === 'notif-updated') fetchCount(); }catch(e){} };
                    }
                }catch(e){}
...
(function(){
    function createLogoutModal(){
        ...
        // Ensure the overlay is above everything (higher than other site z-indexes)
        overlay.style.zIndex = '2147483647';
        // Ensure overlay accepts pointer events so it blocks interaction beneath
        ...
        // Ensure the modal card itself is above the overlay stacking context
        const modalCard = overlay.querySelector('div');
        if (modalCard) {
            modalCard.style.zIndex = '2147483648';
        }
