...
// Count unread notifications excluding those in trash
$readColumn = $hasSecCol ? 'is_read_secretary' : 'is_read';
$sqlNotif = "SELECT COUNT(*) AS count FROM notifications n
             LEFT JOIN notifications_trash t ON (t.notification_id = n.notification_id OR t.notification_id = n.notification_id)
             WHERE t.notification_id IS NULL AND n.$readColumn = 0
             AND NOT (
            ...
            <a href="notifications-secretary.php" class="relative group flex items-center justify-center p-3 rounded-full hover:bg-blue-50 transition-all duration-300" data-tooltip="Notifications">
                <div class="relative">
                    <i class="fas fa-bell text-blue-500 text-lg group-hover:scale-125 group-hover:rotate-6 transition-all duration-300"></i>
                        <?php // Render badge hidden initially; client will fetch authoritative count and show it ?>
...
(function(){
    ...
    const ENDPOINT = '../controllers/notifications_summary.php?role=secretary';
    ...
    function render(count){ if (last===null || last !== count){ last = count; setEl(document.getElementById(BADGE_ID), count); } }
    ...
    document.addEventListener('DOMContentLoaded', function(){ try{ fetchCount(); }catch(e){}; setInterval(fetchCount, POLL_INTERVAL); window.addEventListener('pageshow', function(){ try{ fetchCount(); }catch(e){} }); document.addEventListener('visibilitychange', function(){ if (!document.hidden) { try{ fetchCount(); }catch(e){} } });
        // Cross-tab updates: listen for BroadcastChannel and localStorage fallback
        try{
            if ('BroadcastChannel' in window) {
                const ch = new BroadcastChannel('bpamis-channel');
                ch.onmessage = function(ev){ try{ var d = ev && ev.data ? ev.data : null; if(d && d.type === 'notif-updated' && (!d.role || d.role === 'secretary')) fetchCount(); }catch(e){} };
            }
        }catch(e){}
        window.addEventListener('storage', function(e){ try{ if(!e||!e.key) return; if(e.key==='bpamis-notif-updated'){ var v=null; try{ v=JSON.parse(e.newValue);}catch(_){}; if(v && (!v.role || v.role==='secretary')) fetchCount(); } }catch(e){} });
    });
})();
</script>