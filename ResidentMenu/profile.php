...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_change_password'])) {
    ...
    $pw_query = "SELECT password FROM resident_info WHERE resident_id = ?";
    ...
    if (!password_verify($current_pw, $hashed_pw)) {
        ...
        $update_pw_query = "UPDATE resident_info SET password=? WHERE resident_id=?";
...
$user_name = $_SESSION['resident_name'] ?? 'User Name';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_profile'])) {
    ...
    // Determine the primary email to store. Replace primary if new_email provided.
    // Trim, remove empty entries and normalize indexing so primary extraction is robust.
    $emails_raw = array_values(array_filter(array_map('trim', explode(',', (string)$email)), function($v){ return $v !== ''; }));
    $primary_email = $emails_raw[0] ?? '';
    ...
    if (!empty($new_email)) {
        ...
        if (!filter_var($new_email_trim, FILTER_VALIDATE_EMAIL)) {
            ...
            try {
                // Check officials
                $stmtOff = $conn->prepare("SELECT Official_ID FROM barangay_officials WHERE FIND_IN_SET(?, LOWER(REPLACE(email, ' ', ''))) > 0 LIMIT 1");
                if ($stmtOff) {
                    $stmtOff->bind_param('s', $candidate);
                    $stmtOff->execute();
                    $ro = bpamis_stmt_get_result($stmtOff);
                    if ($ro && $ro->num_rows > 0) {
                ...
                // Check other residents (exclude self)
                if ($profile_error === '') {
                    $stmtRes = $conn->prepare("SELECT resident_id FROM resident_info WHERE resident_id != ? AND FIND_IN_SET(?, LOWER(REPLACE(email, ' ', ''))) > 0 LIMIT 1");
                    if ($stmtRes) {
                        $stmtRes->bind_param('is', $user_id, $candidate);
                        $stmtRes->execute();
                        $rr = bpamis_stmt_get_result($stmtRes);
                        if ($rr && $rr->num_rows > 0) {
                            $profile_error = 'This email address is already used by another resident account.';
                        }
                        $stmtRes->close();
                    } else {
                        $profile_error = 'Failed to verify email uniqueness. Please try again.';
                ...
                if ($profile_error === '') {
                    ...
                    if ($stmtExt) {
                        ...
                        $stmtExt->execute();
                        $re = bpamis_stmt_get_result($stmtExt);
                        if ($re && $re->num_rows > 0) {
                            $profile_error = 'This email address is already used by an external account.';
                        }
                        $stmtExt->close();
                    } else {
                        $profile_error = 'Failed to verify email uniqueness. Please try again.';
    ...
    // Validate birthday if provided (YYYY-MM-DD)
    if ($birthday_in !== '') {
        $dt = DateTime::createFromFormat('Y-m-d', $birthday_in);
        if (!$dt || $dt->format('Y-m-d') !== $birthday_in) {
            $profile_error = 'Please enter a valid birthday date (YYYY-MM-DD).';
        }
    }

    // Enforce minimum age (7 years) if birthday is valid
    if ($birthday_in !== '' && $profile_error === '') {
        $dt2 = DateTime::createFromFormat('Y-m-d', $birthday_in);
        if ($dt2 && $dt2->format('Y-m-d') === $birthday_in) {
    ...
    if ($profile_error === '') {
        if ($first_name && $last_name && $email) {
            $update_query = "UPDATE resident_info SET First_Name=?,Middle_Name=?,Last_Name=?, email=?, Contact_Number=?, Birthdate=? WHERE resident_id =?";
            $stmt = $conn->prepare($update_query);
            $stmt->bind_param("ssssssi", $first_name,$middle_name,$last_name, $email, $contact, $birthday_in, $user_id);
            // Log attempt
            error_log("[resident_profile] Attempting update resident_id={$user_id} email={$email}");
            if ($stmt->execute()) {
                ...
                $_SESSION['user'] = $email;
                // Log and redirect (PRG)
                error_log("[resident_profile] Updated resident_id={$user_id} email={$email}");
                header('Location: profile.php?updated=1');
                exit();
            } else {
                // Log failure details
                error_log("[resident_profile] Failed update resident_id={$user_id} email={$email} - stmt_error=" . ($stmt->error ?? 'N/A') . " conn_error=" . ($conn->error ?? 'N/A'));
...
try {
    ...
    $query = "SELECT * FROM resident_info WHERE resident_id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        // Try multiple possible birthday column names
        foreach (['Birthday','birthday','Date_of_Birth','Birthdate','birthdate','DOB'] as $__b_key) {
            if (!empty($user_data[$__b_key])) { $birthday = $user_data[$__b_key]; break; }
        }
    }
} catch (Exception $e) {
...
while ($row = $hearing_result->fetch_assoc()) {
    $hearings[] = [
        ...
        'link' => 'home-resident.php#calendar'
    ];
}
...
<head>
    ...
    <style>
        ...
        /* Mobile Compact Optimizations */
        @media (max-width: 768px) {
            /* Ensure header doesn't overlap the profile hero on small screens */
            .mobile-header-spacer {
                /* adjust value if header height differs on your layout */
                margin-top: 4.5rem !important;
            }
            /* Flexbox ordering for mobile */
            ...
            .premium-gradient p:first-of-type {
                font-size: 0.75rem !important;
                margin-bottom: 0.5rem !important;
            }
            
            .premium-gradient .space-y-1 p {
            ...
            .premium-card button .w-8,
            .premium-card button .w-10 {
                width: 1.75rem !important;
                height: 1.75rem !important;
            }
            
            .premium-card button i {
                font-size: 0.75rem !important;
            }
            
            .premium-card button span {
                font-size: 0.7rem !important;
            }
            ...
            #dynamic-content .space-y-6 {
                gap: 0.75rem !important;
            }
            
            #dynamic-content .gap-6 {
                gap: 0.75rem !important;
            }
            
            /* Logout Modal */
            #dynamic-content .w-20.h-20 {
                width: 3rem !important;
                height: 3rem !important;
            }
            
            #dynamic-content .text-4xl {
                font-size: 1.5rem !important;
            }
            
            #dynamic-content .text-xl {
                font-size: 0.875rem !important;
            }
...
<body class="bg-gray-50 font-sans min-h-screen">
    ...
    <!-- Main Content -->
    <main class="max-w-7xl mx-auto px-2 sm:px-4 lg:px- py-4 sm:py-8">
    <!-- Profile Hero Section -->
    <div class="mb-8 animate-fade-in mobile-header-spacer">
            <div class="premium-card rounded-3xl overflow-hidden">
                <div class="premium-gradient px-4 sm:px-8 py-8 sm:py-12 relative overflow-hidden rounded-t-3xl">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8">
            ...
            <div class="lg:col-span-2 space-y-6 sm:space-y-8 order-1 lg:order-1">
                <!-- Personal Information Card -->
                ...
                <!-- Mobile-only Quick Actions (appears under Personal Info on small screens) -->
                <div class="premium-card rounded-2xl p-4 sm:p-6 order-2 block lg:hidden">
                    <div class="flex flex-col sm:flex-row items-center sm:items-start mb-4 sm:mb-6">
                ...
                <!-- Dynamic Content Container: selected quick action renders here (below Quick Actions on mobile) -->
                <div id="dynamic-content" class="hidden order-3 lg:order-2 mt-4"></div>
            </div>
            <!-- Sidebar Content -->
            <div class="space-y-6 sm:space-y-8 animate-scale-in order-2 lg:order-2">
                <!-- Quick Actions -->
                <div class="hidden lg:block premium-card rounded-2xl p-4 sm:p-6 order-1">
                    <div class="flex flex-col sm:flex-row items-center sm:items-start mb-4 sm:mb-6">
                        <div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-green-500 to-blue-600 rounded-lg flex items-center justify-center mr-0 sm:mr-3 mb-2 sm:mb-0">
                            <i class="fas fa-cogs text-white text-base sm:text-lg"></i>
                        </div>
                ...
                <!-- Dynamic content is rendered below Personal Information in the main column -->
                
                <!-- Recent Activity -->
                <div class="premium-card rounded-2xl p-4 sm:p-6 order-3">
                    <div class="flex flex-col sm:flex-row items-center sm:items-start mb-4 sm:mb-6">
                    ...
                    <div class="space-y-3 sm:space-y-4">
                        ...
                        <div class="text-center mt-2 sm:mt-4">
                            <a href="home-resident.php#calendar" class="text-blue-600 hover:underline font-semibold text-xs sm:text-base">View More</a>
                        </div>
                    </div>
                </div>
            </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input type="text" name="first_name" 
                                                ...
                                                class="premium-input w-full px-4 py-3 rounded-lg focus:outline-none" required>
                                        </div>

                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-2" required>Middle Name</label>
