...
if(empty($rows)){ echo "<p class='text-center text-red-500'>Case not found.</p>"; exit; }
$case = $rows[0];
$stmt->close();
...
if ($lupon_stmt) {
    ...
    $lupon_stmt->execute();
    $lRows = bpamis_stmt_fetch_all_assoc($lupon_stmt);
    if (!empty($lRows)) {
        $lupon_name = $lRows[0]['lupon_tagapamayapa'] ?? 'Not Yet Assigned';
    }
    $lupon_stmt->close();
}
