...
// Check if user is logged in (use Official_ID stored in session)
if (!isset($_SESSION['official_id'])) {
    header("Location: ../login.php");
    exit();
}

// Use the Barangay Official's ID for the Lupon Head from session
...
$user_role = 'Barangay Lupon Tagapamayapa Head'; // Default role for captain
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_profile'])) {
    ...
    // fetch current email from DB so we only change it when new_email provided.
    $primary_email = '';
    ...
    if (!empty($new_email)) {
        ...
        if (!filter_var($new_email_trim, FILTER_VALIDATE_EMAIL)) {
            ...
            try {
                // Check other officials (exclude current Official_ID)
    ...
    // Validate birthday if provided (expect YYYY-MM-DD)
    if ($birthday_in !== '') {
    ...
    if ($profile_error === '') {
        if ($first_name && $last_name && $email) {
            ...
            if ($stmt->execute()) {
                ...
                // Use Post-Redirect-Get so the updated email displays immediately and avoids form resubmission warnings
                $_SESSION['official_name'] = $full_name;
                $_SESSION['user'] = $email;
                header('Location: profile_luponhead.php?updated=1');
                exit();
            } else {
...
try {
    // Query to get user information from barangay_officials table using the logged-in Official_ID
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // Fallback: fetch the Lupon Head record by Position (for safety if session lacks id)
        $fallbackSql = "SELECT * FROM barangay_officials WHERE Position = 'Lupon Tagapamayapa Head' ORDER BY Official_ID LIMIT 1";
        $fallbackRes = $conn->query($fallbackSql);
        if ($fallbackRes && $fallbackRes->num_rows > 0) {
            $user_data = $fallbackRes->fetch_assoc();
            $user_id = (int)($user_data['Official_ID'] ?? $user_id);
        }
    }
    ...
    if ($user_data) {
        ...
        // Derive birthday from a variety of possible column names
    ...
    // Determine avatar source: prefer user-provided fields, else fall back to default choices
    $avatar_src = '';
    if (!empty($user_data)) {
        // Common possible column names for stored avatar/photo
        foreach (['avatar','photo','profile_pic','Profile_Pic','photo_url','picture'] as $col) {
            if (!empty($user_data[$col])) { $avatar_src = trim((string)$user_data[$col]); break; }
        }
        // If avatar appears as a base64 data URL or absolute URL, keep it; else try to build a relative path
        if ($avatar_src !== '') {
            // nothing to do; assume stored path is valid
        }
    }
    if (empty($avatar_src)) {
        // Build two-letter initials from the user's name for the placeholder avatar
        $initials = 'BP';
        if (!empty($user_name)) {
            ...
            if (count($parts) >= 2) {
                ...
                $initials = strtoupper(mb_substr($parts[0], 0, 2, 'UTF-8'));
            }
            // Ensure exactly two characters
            $initials = mb_substr($initials, 0, 2, 'UTF-8');
        }
        // Use ui-avatars service to generate a two-letter avatar matching other profiles
        $avatar_src = 'https://ui-avatars.com/api/?name=' . urlencode($initials) . '&background=667eea&color=fff&size=128&length=2';
    }

    // Override role display for Lupon Head profile to match requested format
    $user_role = 'Lupon Tagapamayapa Hepe [Conciliation Panel]';
} catch (Exception $e) {
...
try {
    ...
    while ($row = $ongoingResult->fetch_assoc()) {
        ...
        $recent_activities[] = [
            ...
            'date' => $opened,
            'link' => 'home-luponhead.php#calendar'
        ];
    }
} catch (Exception $e) {
    ...
    <style>
        ...
        @media (max-width: 640px) {
            ...
            * {
            ...
            .premium-gradient .text-base {
                font-size: 0.7rem !important;
            }
            
            .premium-gradient .text-xs {
                font-size: 0.625rem !important;
            }
            
            /* Grid spacing */
            .grid.gap-4, .grid.gap-8 {
            ...
            .premium-card .w-10, .premium-card .w-12 {
            ...
            /* Personal info grid */
            .premium-card .grid.gap-6 {
                gap: 0.5rem !important;
            }
            
            .premium-card .space-y-3, .premium-card .space-y-4 {
                gap: 0.25rem !important;
            }
            
            .premium-card .py-2, .premium-card .py-3 {
            ...
            #dynamic-content .bg-gradient-to-r {
                padding: 0.75rem 1rem !important;
            }
            
            #dynamic-content .bg-gradient-to-r h3 {
                font-size: 0.9rem !important;
            }
            ...
            #dynamic-content .text-sm {
                font-size: 0.7rem !important;
            }
            
            #dynamic-content .text-xs {
                font-size: 0.65rem !important;
            }
            ...
            #dynamic-content .grid.gap-4, #dynamic-content .grid.gap-6 {
                gap: 0.5rem !important;
            }
            
            /* Stat cards - if present */
            .stat-card {
                padding: 0.75rem !important;
            }
...
<body class="font-sans text-gray-700">
    ...
    <main class="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-8 relative z-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8">
            ...
            <div class="lg:col-span-2 space-y-6 sm:space-y-8">
                ...
                <!-- Mobile Quick Actions (placed under Personal Information on small screens) -->
                <div class="premium-card rounded-2xl p-4 sm:p-6 block lg:hidden">
                    <div class="flex flex-col sm:flex-row items-center sm:items-start mb-4 sm:mb-6">
                    ...
                    <div class="space-y-2 sm:space-y-3">
                        <button onclick="showContent('edit-profile')" class="w-full flex items-center p-3 sm:p-4 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-300 group">
                            <div class="w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 rounded-lg flex items-center justify-center mr-2 sm:mr-3 group-hover:bg-blue-200 transition-colors">
                                <i class="fas fa-user-edit text-blue-700 text-sm"></i>
                            </div>
                        ...
                        <button onclick="showContent('change-password')" class="w-full flex items-center p-3 sm:p-4 rounded-xl hover:bg-gradient-to-r hover:from-green-50 hover:to-blue-50 transition-all duration-300 group">
                            <div class="w-8 h-8 sm:w-10 sm:h-10 bg-green-100 rounded-lg flex items-center justify-center mr-2 sm:mr-3 group-hover:bg-green-200 transition-colors">
                                <i class="fas fa-key text-green-700 text-sm"></i>
                            </div>
                    ...
                    <div class="space-y-3 sm:space-y-4">
                        <?php if (!empty($recent_activities)) : ?>
                            ...
                            <div class="text-center mt-2 sm:mt-4">
                                <a href="home-luponhead.php#calendar" class="text-blue-600 hover:underline font-semibold text-xs sm:text-base">View More</a>
                            </div>
                        <?php else: ?>
    ...
    <script>
                            ...
                            <div class="p-6">
                                ...
                                <form class="space-y-6" method="post" action="">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="address" value="<?= htmlspecialchars($address) ?>" placeholder="Street/Purok num House number" class="premium-input w-full px-4 py-3 rounded-lg focus:outline-none">
                                    </div>
                                    <div>
        ...
        function attachBarangayInfoHandlers() {
            ...
            fetch('profile_luponhead.php', { method: 'POST', body: loadData })
                .then(r => r.json())
                .then(j => {
                    if (j.success && j.data) {
            ...
            form.addEventListener('submit', function(e){
                ...
                fetch('profile_luponhead.php', { method: 'POST', body: fd })
                    .then(r => r.json())
                    .then(j => {
                        if (j.success) {
                            msgDiv.className = 'mt-2 p-3 bg-green-100 text-green-800 rounded-lg text-center';
                            msgDiv.innerHTML = (j.message || 'Saved.') + ' <a href="../bpamis_website/bpamis.php" target="_blank" class="underline font-medium">Open homepage</a>';
                            ...
                            msgDiv.className = 'mt-2 p-3 bg-red-100 text-red-800 rounded-lg text-center';
                            msgDiv.textContent = j.message || 'Failed to save.';
                        }
                    })
                    .catch(() => {
                        msgDiv.className = 'mt-2 p-3 bg-red-100 text-red-800 rounded-lg text-center';
