...
// Enforce Captain-only access within Official namespace
...
if (!isset($_SESSION['official_id']) || !is_numeric($_SESSION['official_id'])) {
    header("Location: {$loginPage}");
    exit();
}
// Must be Barangay Captain specifically
$__position = strtolower(trim($_SESSION['official_position'] ?? ''));
if (strpos($__position, 'barangay captain') === false) {
    // Redirect based on position if we can infer a better destination
    if (strpos($__position, 'secretary') !== false) {
        header("Location: ../SecMenu/home-secretary.php");
        exit();
    }
    if (strpos($__position, 'lupon') !== false) {
        if (strpos($__position, 'hepe') !== false || strpos($__position, 'head') !== false) {
            header("Location: ../LuponHeadMenu/home-luponhead.php");
            exit();
        }
        header("Location: ./home-lupon.php");
        exit();
    }
    // Fallback: send to login/landing if position is unknown
    header("Location: {$loginPage}");
    exit();
}

$user_id = (int)($_SESSION['official_id']);
...
$user_role = 'Barangay Captain'; // Default fallback, will be replaced by actual Position from database
$birthday = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_profile'])) {
    ...
    if (!empty($new_email)) {
        ...
        if (!filter_var($new_email_trim, FILTER_VALIDATE_EMAIL)) {
            ...
            // Check uniqueness across barangay_officials (exclude self), resident_info, external_complainant
            try {
                // officials
                $q1 = "SELECT Official_ID FROM barangay_officials WHERE Official_ID != ? AND FIND_IN_SET(?, LOWER(REPLACE(email, ' ', ''))) > 0 LIMIT 1";
                $stmtChk = $conn->prepare($q1);
                $lowEmail = strtolower($new_email_trim);
                $stmtChk->bind_param('is', $user_id, $lowEmail);
                $stmtChk->execute();
                $resChk = bpamis_stmt_get_result($stmtChk);
                if ($resChk && $resChk->num_rows > 0) {
                    $profile_error = 'That email address is already in use by another official.';
                }
                $stmtChk->close();
                // resident_info
                if ($profile_error === '') {
                    $q2 = "SELECT resident_id FROM resident_info WHERE FIND_IN_SET(?, LOWER(REPLACE(email, ' ', ''))) > 0 LIMIT 1";
                    $stmtChk2 = $conn->prepare($q2);
                    $stmtChk2->bind_param('s', $lowEmail);
                    $stmtChk2->execute();
                    $resChk2 = bpamis_stmt_get_result($stmtChk2);
                    if ($resChk2 && $resChk2->num_rows > 0) {
                        $profile_error = 'That email address is already used by a resident account.';
                    }
                    $stmtChk2->close();
                }
                // external_complainant
                if ($profile_error === '') {
                    $q3 = "SELECT external_complaint_id FROM external_complainant WHERE FIND_IN_SET(?, LOWER(REPLACE(email, ' ', ''))) > 0 LIMIT 1";
                    $stmtChk3 = $conn->prepare($q3);
                    $stmtChk3->bind_param('s', $lowEmail);
                    $stmtChk3->execute();
                    $resChk3 = bpamis_stmt_get_result($stmtChk3);
                    if ($resChk3 && $resChk3->num_rows > 0) {
                        $profile_error = 'That email address is already used by an external account.';
                    }
                    $stmtChk3->close();
                }
            } catch (Exception $e) {
                error_log('Email uniqueness check error: ' . $e->getMessage());
                if ($profile_error === '') $profile_error = 'Failed to validate new email. Please try again.';
            }

            if ($profile_error === '') {
                // Accept new email as primary
    ...
    if ($profile_error === '') {
        if ($first_name && $last_name) {
            ...
            if ($stmt->execute()) {
                ...
                $_SESSION['official_name'] = $full_name;
                $_SESSION['user'] = $email;
                header('Location: profile-captain.php?updated=1');
                exit();
            } else {
...
try {
    ...
    while ($row = $ongoingResult->fetch_assoc()) {
        ...
        $recent_activities[] = [
            ...
            'date' => $opened,
            'link' => 'home-captain.php#calendar'
        ];
    }
} catch (Exception $e) {
                    ...
                    <div class="space-y-3 sm:space-y-4">
                        <?php if (!empty($recent_activities)) : ?>
                            ...
                            <div class="text-center mt-2 sm:mt-4">
                                <a href="home-captain.php#calendar" class="text-blue-600 hover:underline font-semibold text-xs sm:text-base">View More</a>
                            </div>
                        <?php else: ?>
