...
<head>
    ...
    <style>
        ...
        @media (max-width: 640px) {
            ...
            /* Remove colgroup constraints on mobile */
            .premium-table colgroup {
                display: none !important;
            }
            
            /* Give each column breathing room and proper min-widths */
            ...
            <div class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="flex flex-col justify-end">
                    <div class="flex gap-2">
                        ...
                        <form action="export_pdf.php" method="post" target="_blank" id="pdfForm" class="flex-1">
            ...
            <table class="premium-table table-fixed w-full mt-4 border-collapse rounded-lg overflow-hidden">
                ...
                <thead>
                    <tr>
                        ...
                        <th class="text-left">Resolution Time</th>
...
ORDER BY c.Date_Opened DESC";
                ...
                <a href="view_case_details_cap.php?id=' . urlencode($case['Case_ID']) . '" class="card-hover bg-primary-500 text-white px-3 py-1.5 rounded-lg hover:bg-primary-600 transition inline-flex items-center text-sm">
        ...
        <script>
            ...
            function applyStatusFilter(){
                ...
                rows.forEach(tr => {
                    ...
                    if(selectedText && selectedText !== 'all statuses'){
                        // Synonym mapping: Conciliation ≈ Resolution/Resolved
                        if(selectedText === 'conciliation'){
                            show = /conciliation|resolution|resolved/.test(statusText);
                        } else {
            ...
            function fetchFilteredData() {
                // Use client-side filtering only (no server fetch)
                applyStatusFilter();
            }

            document.getElementById('caseStatus').addEventListener('change', applyStatusFilter);
            document.getElementById('dateRange').addEventListener('change', applyStatusFilter);
            ...
            function attachRowClickHandlers() {
                ...
                tableRows.forEach(row => {
                    row.addEventListener('click', function(e) {
                        ...
                        const viewLink = this.querySelector('a[href*="view_case_details_cap.php"]');
                        if (viewLink) {
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const colorMap = (label) => {
                    ...
                    if (s.includes('mediation resolved')) return 'rgba(16, 185, 129, 0.7)'; // green
                    if (s.includes('conciliation resolved') || s.includes('resolution resolved')) return 'rgba(16, 185, 129, 0.7)'; // green
                    if (s.includes('arbitration resolved')) return 'rgba(16, 185, 129, 0.7)'; // green
                    if (s.includes('certificate to file action')) return 'rgba(96, 87, 87, 0.7)'; // red
                    if (s.includes('dismissed')) return 'rgba(239, 68, 68, 0.7)'; // red
                    if (s.includes('pending')) return 'rgba(245, 158, 11, 0.7)'; // amber
                    if (s.includes('mediation')) return 'rgba(168, 85, 247, 0.7)'; // purple
                    if (s.includes('conciliation') || s.includes('resolution')) return 'rgba(234, 179, 8, 0.7)'; // yellow
                    if (s.includes('arbitration')) return 'rgba(59, 130, 246, 0.7)'; // blue
                    if (s.includes('resolved') || s.includes('closed')) return 'rgba(16, 185, 129, 0.7)'; // green
                    if (s.includes('open')) return 'rgba(59, 130, 246, 0.7)'; // blue
