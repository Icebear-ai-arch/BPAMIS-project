...
if ($res = $conn->query("SHOW COLUMNS FROM CASE_INFO LIKE 'Certificate_Path'")) { if ($res->num_rows > 0) $hasCertificateCol = true; $res->close(); }
...
if ($hasCertificateCol) {
    $cst = $conn->prepare('SELECT Certificate_Path FROM CASE_INFO WHERE Case_ID = ? LIMIT 1');
    if ($cst) {
...
if ($stmt_main_id) {
    ...
    $q = $conn->query("SELECT Respondent_ID FROM COMPLAINT_INFO WHERE Complaint_ID = " . (int)$complaint_id);
...
if ($main_respondent_id) {
    $stmt_main_name = $conn->prepare("SELECT First_Name,Last_Name FROM RESIDENT_INFO WHERE Resident_ID=?");
    if ($stmt_main_name) {
        ...
        $q = $conn->query("SELECT First_Name,Last_Name FROM RESIDENT_INFO WHERE Resident_ID = " . (int)$main_respondent_id);
        if ($q) {
...
if ($stmt_others) {
    ...
    $stmt_others->execute();
    $res_o = bpamis_stmt_get_result($stmt_others);
    while ($row = $res_o->fetch_assoc()) { $respondent_names[] = $row['First_Name'] . ' ' . $row['Last_Name']; }
    $stmt_others->close();
} else {
    $q = $conn->query("SELECT ri.First_Name,ri.Last_Name FROM COMPLAINT_RESPONDENTS cr JOIN RESIDENT_INFO ri ON cr.Respondent_ID=ri.Resident_ID WHERE cr.Complaint_ID = " . (int)$complaint_id);
...
$stmt_logs = $conn->prepare("SELECT ml.*, sl.hearingTitle FROM MEETING_LOGS ml LEFT JOIN schedule_list sl ON sl.Case_ID = ml.Case_ID AND DATE(sl.HearingDateTime) = ml.Hearing_Date AND TIME(sl.HearingDateTime) = ml.Hearing_Time WHERE ml.Case_ID = ? ORDER BY ml.Hearing_Date DESC, ml.Hearing_Time DESC, ml.Log_ID DESC");
$meetingLogs = [];
if ($stmt_logs) {
    ...
    $logsResult = $conn->query("SELECT ml.*, sl.hearingTitle FROM MEETING_LOGS ml LEFT JOIN schedule_list sl ON sl.Case_ID = ml.Case_ID AND DATE(sl.HearingDateTime) = ml.Hearing_Date AND TIME(sl.HearingDateTime) = ml.Hearing_Time WHERE ml.Case_ID = " . (int)$case_id . " ORDER BY ml.Hearing_Date DESC, ml.Hearing_Time DESC, ml.Log_ID DESC");
    while ($row = $logsResult->fetch_assoc()) {
        $meetingLogs[] = $row;
    }
}
...
$feedback_stmt = $conn->prepare("SELECT * FROM feedback WHERE case_id = ? ORDER BY created_at DESC");
$feedbackList = [];
if ($feedback_stmt) {
...
<head>
    ...
    <link rel="icon" type="image/png" href="/BPAMIS/OfficialMenu/logo.png" />
